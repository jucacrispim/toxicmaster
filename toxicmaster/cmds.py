...
from toxiccore.utils import (daemonize as daemon,
                             bcrypt_string, changedir, set_loglevel, log)
from toxicmaster import (
    ENVVAR,
    DEFAULT_SETTINGS,
...
@command
def create(root_dir, secrets_token='', poller_token='', no_token=False):
    """ Creates a new toxicmaster environment.

    :param --root_dir: Root directory for toxicmaster.
    :param --secrets-token: The auth token for the secrets server.
    ...
    content = content.replace('{{SECRETS_TOKEN}}', secrets_token)
...
def run(loglevel):
    ...
    from toxicmaster.hole import HoleServer
...
async def toxicinit(server):
    ...
    from toxicmaster.build import BuildSet
    from toxicmaster.consumers import RepositoryMessageConsumer
    from toxicmaster.repository import Repository
    from toxicmaster.slave import Slave
    from toxicmaster.users import User, Organization
